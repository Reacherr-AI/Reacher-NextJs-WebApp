...
export async function POST(req: Request) {
  ...
  const storedTarget = store.get(PHONE_TARGET_COOKIE)?.value;
  const target = body?.target ?? storedTarget;

  if (body?.otpType === 'PHONE') {
    if (!target) {
      return NextResponse.json(
        { message: 'target is required for PHONE otpType' },
        { status: 400 }
      );
    }
    body.target = target;
  ...
  const res = await fetch(`${BACKEND_URL}/api/v1/verify/phone`, {
    method: 'POST',
